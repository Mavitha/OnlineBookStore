...
<head>
  ...
  <title>Lumina Books — Catalog</title>
  ...
  <header class="site-header" id="siteHeader"><header class="site-header" id="siteHeader">
    ...
    <div class="header-subsearch">
      <div class="container">
        ...
        <form method="GET" action="catalog.php" class="search-filter-composite" style="display:flex; flex-wrap:wrap; gap:15px; width: 100%;">
          <!-- Search Input -->
          <div class="search-input-box" style="flex:1; min-width: 250px;">
            ...
            <input type="text" name="search" placeholder="Search by title, author..." autocomplete="off" value="<?= htmlspecialchars($search) ?>">
          </div>
          ...
          <div class="filter-box">
            <select name="sort" class="filter-select">
              ...
              <option value="title-az" <?= $sort === 'title-az' ? 'selected' : '' ?>>Title: A to Z</option>
            </select>
          </div>
...
<!-- ================= VIEW 5: BROWSE ALL FULL CATALOG ================= -->
    <section id="view-browse" class="app-view">
      <div class="page-banner">
        <div class="container">
          <div class="breadcrumb">
            <a href="#home">Home</a>
            <span class="breadcrumb-sep">/</span>
            <span class="breadcrumb-current">Complete Library</span>
          </div>
          <h1 class="page-title">Browse Full Library</h1>
          <p class="page-subtitle">Browse page by page through our entire literary catalog until the very last volume.</p>
        </div>
      </div>

      <!-- Upper Search Bar for Browse Catalog -->
      <div class="container view-toolbar">
        <div class="toolbar-search-box">
          ...
          <input type="text" id="browseSearchInput" placeholder="Search entire catalogue..." autocomplete="off">
        </div>
        <div class="toolbar-stats" id="browseResultsCount">
          Showing all books
        </div>
      </div>
      ...
      <div class="container">
        ...
        <div class="books-grid" id="browseFullGrid">
            <?php if (count($books) === 0): ?>
                <p style="grid-column: 1/-1; text-align: center;">No books found matching your criteria.</p>
            <?php else: ?>
                <?php foreach ($books as $book): ?>
                <article class="book-card">
                  <div class="book-cover-wrap">
                    <img class="book-cover-img" src="../backend/images/<?= htmlspecialchars($book['cover_image']) ?>" alt="<?= htmlspecialchars($book['title']) ?>" onerror="this.src='https://images.unsplash.com/photo-1544947950-fa07a98d237f?auto=format&fit=crop&w=600&q=80'">
                  </div>
                  <div class="book-info">
                    ...
                    <div class="book-card-bottom">
                      <span class="book-price">$<?= htmlspecialchars(number_format($book['price'], 2)) ?></span>
                      <form method="POST" action="cart.php" style="margin:0;">
                        <input type="hidden" name="action" value="add">
                        <input type="hidden" name="id" value="<?= $book['id'] ?>">
                        <input type="hidden" name="title" value="<?= htmlspecialchars($book['title']) ?>">
                        <input type="hidden" name="price" value="<?= $book['price'] ?>">
                        <button type="submit" class="add-cart-mini-btn" title="Add to Bag">
        ...
        <!-- Multi-page numbered pagination until books finish -->
        <div class="pagination-container" id="browseFullPagination">
          <!-- Rendered by JS -->
        </div>
      </div>
