...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['login'])) {
        ...
        if (isset($_POST['add_book'])) {
            $stmt = $pdo->prepare("INSERT INTO books (title, author, category, price, cover_image) VALUES (?, ?, ?, ?, 'default.jpg')");
            $stmt->execute([$_POST['title'], $_POST['author'], $_POST['category'], $_POST['price']]);
            header("Location: admin_dashboard.php");
            exit;
...
<head>
  ...
  <title>Lumina Books — Admin Dashboard</title>
  ...
  <header class="site-header" id="siteHeader"><header class="site-header" id="siteHeader">
    <div class="header-main container">
      ...
      <nav class="desktop-nav">
        <a href="index.html" class="nav-link" data-view="home">Home</a>
  ...
  <main class="app-main-content">
    ...
    <section class="app-view active">
      ...
      <div class="container" style="padding: 40px 0;">
        <?php if (!$is_admin): ?>
            <div style="max-width: 400px; margin: 0 auto; background: var(--bg-surface); padding: 30px; border-radius: 12px; border: 1px solid var(--border-color);">
                <h3>Admin Login</h3>
                <?php if($error): ?><p style="color:red;"><?= htmlspecialchars($error) ?></p><?php endif; ?>
                <form method="POST" action="admin_dashboard.php" style="display: flex; flex-direction: column; gap: 15px; margin-top: 15px;">
                    <input type="email" name="email" placeholder="Admin Email" required style="padding: 8px;">
        ...
        <?php else: ?>
            ...
            <!-- Add Book Form -->
            <div style="background: var(--bg-surface); padding: 20px; border-radius: 8px; margin-bottom: 30px; border: 1px solid var(--border-color);">
                <h4>Add New Book</h4>
                <form method="POST" action="admin_dashboard.php" style="display: flex; gap: 10px; margin-top: 10px; flex-wrap: wrap;">
                    <input type="text" name="title" placeholder="Title" required style="padding: 8px; flex:1; min-width:150px;">
                    <input type="text" name="author" placeholder="Author" required style="padding: 8px; flex:1; min-width:150px;">
                    <input type="text" name="category" placeholder="Category" required style="padding: 8px; flex:1; min-width:100px;">
                    <input type="number" name="price" placeholder="Price" step="0.01" required style="padding: 8px; flex:1; min-width:80px;">
            ...
            <table style="width: 100%; text-align: left; border-collapse: collapse; margin-bottom: 40px;">
                <thead>
                    <tr style="border-bottom: 2px solid var(--border-color);">
                        <th style="padding: 10px;">ID</th><th style="padding: 10px;">Title</th><th style="padding: 10px;">Author</th><th style="padding: 10px;">Price</th><th style="padding: 10px;">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid var(--border-color);">
                        ...
                        <td style="padding: 10px;"><?= htmlspecialchars($book['title']) ?></td>
                        <td style="padding: 10px;"><?= htmlspecialchars($book['author']) ?></td>
                        <td style="padding: 10px;">$<?= number_format($book['price'], 2) ?></td>
                        <td style="padding: 10px;">
                            <form method="POST" action="admin_dashboard.php" style="margin:0;">
                                <input type="hidden" name="id" value="<?= $book['id'] ?>">
                                <button type="submit" name="delete_book" class="btn btn-sm" style="color:red; border:1px solid red; background:none;">Delete</button>
                            </form>
                        </td>
                    </tr>
                    <?php endforeach; ?>
            ...
            <table style="width: 100%; text-align: left; border-collapse: collapse;">
                <thead>
                    <tr style="border-bottom: 2px solid var(--border-color);">
                        <th style="padding: 10px;">Order ID</th><th style="padding: 10px;">Customer</th><th style="padding: 10px;">Book ID</th><th style="padding: 10px;">Price</th><th style="padding: 10px;">Date</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid var(--border-color);">
                        ...
                        <td style="padding: 10px;"><?= htmlspecialchars($order['customer_name']) ?><br><small><?= htmlspecialchars($order['customer_email']) ?></small></td>
                        <td style="padding: 10px;"><?= $order['book_id'] ?></td>
                        <td style="padding: 10px;">$<?= number_format($order['total_price'], 2) ?> (<?= htmlspecialchars($order['payment_method']) ?>)</td>
