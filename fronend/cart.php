...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'add') {
        ...
        $title = $_POST['title'];
        $price = $_POST['price'];
        $_SESSION['cart'][] = ['id' => $id, 'title' => $title, 'price' => $price];
        ...
        if (!empty($_SESSION['cart'])) {
            try {
                ...
                foreach ($_SESSION['cart'] as $item) {
                    $stmt = $pdo->prepare("INSERT INTO orders (book_id, customer_name, customer_email, total_price, payment_method, payment_status) VALUES (?, ?, ?, ?, ?, 'Completed')");
                    $stmt->execute([$item['id'], $name, $email, $item['price'], $payment]);
                }
                $pdo->commit();
                $_SESSION['cart'] = [];
  ...
  <main class="app-main-content">
    ...
    <section class="app-view active">
      ...
      <div class="container" style="padding: 40px 0;">
        <?php if ($message): ?>
            <div style="background: #e6ffe6; padding: 15px; border: 1px solid #00cc00; margin-bottom: 20px; border-radius: 8px;">
        ...
        <div style="background: var(--bg-surface); padding: 30px; border-radius: 12px; border: 1px solid var(--border-color);">
            ...
            <?php else: ?>
                ...
                <div class="cart-summary-lines">
                    ...
                    <div class="summary-line summary-total"><span>Total</span><span>$<?= number_format($subtotal, 2) ?></span></div>
                </div>

                <div style="margin-top: 30px; padding-top: 20px; border-top: 2px solid #eee;">
                    <h3>Checkout Details</h3>
                    <form method="POST" action="cart.php" style="display: flex; flex-direction: column; gap: 10px; margin-top: 10px;">
                        <input type="hidden" name="action" value="checkout">
                        <input type="text" name="name" placeholder="Full Name" required style="padding: 8px; border-radius: 4px; border: 1px solid #ccc;" />
                        <input type="email" name="email" placeholder="Email Address" required style="padding: 8px; border-radius: 4px; border: 1px solid #ccc;" />
                        <select name="payment" style="padding: 8px; border-radius: 4px; border: 1px solid #ccc;">
                            <option value="Credit Card">Credit Card</option>
                            <option value="PayPal">PayPal</option>
                            <option value="Bank Transfer">Bank Transfer</option>
                        </select>
                        <button type="submit" class="btn btn-primary" style="margin-top: 20px;">Complete Purchase</button>
                    </form>
                </div>
            <?php endif; ?>
        </div>
